...
const EditBlog = () => {
  ...
  const submitEditBlog = async (e) => {
    ...
    if (userinfo.title === "" || des === "" || userinfo.categoryId === "") {
    ...
    try {
      const { data } = await axios.put(
        `${import.meta.env.VITE_BACKEND_URL}blogs/updateBlog`,
        formData
      );
    ...
    <Container>
      <form onSubmit={submitEditBlog}>
        <div className="login-con p-0">
          ...
          >
            <div className="mt-4">
              <div className="text-xl">Previous cover Photo</div>
            ...
            {userinfo.editImg ? (
              <div className="mt-4">
                <div className="text-xl">New cover Photo</div>
                <div className="h-[100px] w-full border border-gray-300 rounded-xl p-2 mt-2">
                  <img
                    src={URL.createObjectURL(userinfo.editImg)}
                    className="object-contain w-full h-full"
                  />
                </div>
              </div>
          ...
          <div
            ...
            style={{ border: "1px solid #5e5e5e4b" }}
          >
            <LoadingBtn name={"Edit"} load={load} />
          </div>
        </div>
      </form>
    </Container>
